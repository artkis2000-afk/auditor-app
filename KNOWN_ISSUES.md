...
## KI-17 — GET /api/invoices: невалидный ?status → 400 (в legacy — пустой список)
- **Статус:** RESOLVED (осознанное отличие от legacy, согласовано — требование «invalid query → 400»)
- **Где (исходник):** GET /api/invoices — legacy фильтровал по строке status без валидации
  (неизвестный status просто давал пустой результат).
- **Решение в v2:** транспорт валидирует query через `invoicesListQuerySchema`
  (status ∈ InvoiceStatus). Неизвестный status → 400, а не пустой список. Схемы не ослаблялись.
