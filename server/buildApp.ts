...
import { loadFirebaseAuthConfig } from './auth/index.js';
...
export function buildAppFromEnv(): Express {
  const authConfig = loadFirebaseAuthConfig(); // FIREBASE_PROJECT_ID + AUTH_ADMIN_EMAILS (keyless, без секрета)
  ...
  return composeApp(gateway, authConfig, { ocr, imageStore, corsOrigins });
}
